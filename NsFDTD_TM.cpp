...
bool NsFDTD_TM::calc() {
	...
	CalcE();	//電場の計算
	CalcE_PML();
	NsScatteredWave(wave_angle);	//散乱波の入射0
	//pointLightSource(Ez);
	//absorbing();
	//pointLightSource(Ez);
	//NsPointLightSource();
...
bool NsFDTD_TM::EndTask(){
	...
	string label = "";
	NTFFindexform("", NTFF::NTFFDATA | NTFF::TOTAL | NTFF::REFLEC | NTFF::FULL);	// label -> "" にしたけど動くか確認してない.
...
void NsFDTD_TM::field(){		
	...
	R_M = NsCoef();		//差分演算用の計算定数の設定，小r,具体更新公式在nsfdtdTM.h
	
	R_P = 1.0-R_M;
	//R_P = NsCoef();		//差分演算用の計算定数の設定，小r,具体更新公式在nsfdtdTM.h
	...
	for(int i=0; i<mField->getNpx(); i++){
		for(int j=0; j<mField->getNpy(); j++){
			...
			CEZLH(i,j) = u_ez*sqrt(mu/EPSEZ(i,j));		//u' √(μ/ε)Dongzhu P44
