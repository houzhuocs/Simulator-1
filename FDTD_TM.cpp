...
#pragma omp parallel for
	...
	for(int i=0; i<mField->getNpx(); i++){
		for(int j=0; j<mField->getNpy(); j++){
			EPSEZ(i,j) = mModel->calcEPS(i,j);//求epsilon
...
void FDTD_TM::NsPointLightSource() {
	for (int i = mField->getNpml(); i < mField->getNpx() - mField->getNpml(); i++) {
		for (int j = mField->getNpml(); j < mField->getNpy() - mField->getNpml(); j++) {
			if (EPSEZ(i, j) == EPSILON_0_S) continue;
			double n = sqrt(EPSEZ(i, j) / EPSILON_0_S);
			EZX(mField->getNpml(), mField->getNpy() / 2, +1) += (1.0 / (n)-1)*0.5*polar(1.0, w_s * (time + DT_S));
			EZY(mField->getNpml(), mField->getNpy() / 2, +1) += (1.0 / (n)-1)*0.5*polar(1.0, w_s * (time + DT_S));
			EZ(mField->getNpml(), mField->getNpy() / 2, +1) += (1.0 / (n)-1)*ray_coef * polar(1.0, w_s*time);
		}
	}
}
//���˔g
...
void FDTD_TM::NTFFindexform(string label, NTFF::output flag){
	...
	if( (flag & NTFF::TOTAL) == NTFF::TOTAL){
		...
		//fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << ")  " << sum<< ',' << endl;		//�g�����Ƃ̑��a��o��
		//fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << "sum)  " << sum << ','<< endl;
		//fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << "sum180)  " << sum180 << ',' << endl;
		fp  << sum180 << ',' << endl;
		//fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << "sum/sumTR)  " << sum/sumTR << ',' << endl;
		//fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << "sum180 / sumTR)  " << sum180 / sumTR << ',' << endl;
		//fp << sum << ',' << endl;
	}
	...
	*/
	if ((flag & NTFF::FULL) == NTFF::FULL) {
	
		ofstream fp = WriteOpen(MakeDir("NTFF") + label + "WaveAngleStrength_full", DATAFILE::ADD); //false�͒ǋL���[�h
	
		fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << ")  " << sum180 << endl;
		
	
	}
	if ((flag & NTFF::REFLEC) == NTFF::REFLEC) {

		ofstream fp = WriteOpen(MakeDir("NTFF") + label + "Reflectence", DATAFILE::ADD); //false�͒ǋL���[�h

		fp << "(" << to_s((int)Inv_Nano_S(lambda_s)) << "," << wave_angle << ")  " << sum180/ sumTR << ","  << endl;
